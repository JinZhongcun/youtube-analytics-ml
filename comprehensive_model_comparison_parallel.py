...
models = {
    ...
    'LightGBM': lgb.LGBMRegressor(random_state=42, verbosity=-1, n_jobs=n_jobs),
    'XGBoost': xgb.XGBRegressor(random_state=42, verbosity=0, n_jobs=n_jobs)
}
...
with ProcessPoolExecutor(max_workers=len(models)) as executor:
    ...
    for name, model in models.items():
        future = executor.submit(evaluate_model, name, model, 
                               X_train_scaled, y_train, X_test_scaled, y_test, cv)
        futures.append(future)
    
    for future in as_completed(futures):
        name, result, model = future.result()
        model_results[name] = result
        trained_models[name] = model

# PCA + 各モデルの評価
print("\n" + "="*60)
print("PCA + 予測モデルの組み合わせ（並列処理）")
print("="*60)
...
with ProcessPoolExecutor(max_workers=len(models)) as executor:
    futures = []
    for name, model in models.items():
        future = executor.submit(evaluate_model, f"PCA + {name}", model, 
                               X_train_pca, y_train, X_test_pca, y_test, cv)
        futures.append(future)
    
    for future in as_completed(futures):
        name, result, model = future.result()
        pca_model_results[name] = result

total_elapsed_time = time.time() - total_start_time
print(f"\n全モデルの評価完了！総処理時間: {total_elapsed_time:.2f}秒")
...
print("\n順位 | モデル名                   | CV R² (平均±SD)  | テスト R² | 処理時間")
print("-" * 80)
for i, (name, metrics) in enumerate(sorted_models):
    cv_score = f"{metrics['cv_r2_mean']:.4f} ± {metrics['cv_r2_std']:.4f}"
    print(f"{i+1:2d}   | {name:25s} | {cv_score:15s} | {metrics['test_r2']:.4f}   | {metrics['elapsed_time']:.2f}秒")
...
for name in ['Random Forest', 'LightGBM', 'XGBoost', 'SVM (RBF)', 'Linear Regression']:
    if name in models:
        model = models[name]
        scores = cross_val_score(model, X_train_scaled, y_train, cv=cv, scoring='r2', n_jobs=n_jobs)
...
# 特徴量の重要度（LightGBM）
plt.subplot(3, 2, 3)
lgb_model = lgb.LGBMRegressor(random_state=42, verbosity=-1, n_jobs=n_jobs)
...
# 処理時間の比較
plt.subplot(3, 2, 4)
model_names_time = list(model_results.keys())
processing_times = [model_results[name]['elapsed_time'] for name in model_names_time]
plt.bar(model_names_time, processing_times)
plt.xlabel('Model')
...
# 訓練vsテストR²の散布図
plt.subplot(3, 2, 5)
train_r2s = [metrics['train_r2'] for _, metrics in all_results.items()]
test_r2s = [metrics['test_r2'] for _, metrics in all_results.items()]
plt.scatter(train_r2s, test_r2s, alpha=0.6, s=100)
plt.plot([0, max(train_r2s)], [0, max(train_r2s)], 'r--', lw=2)
plt.xlabel('Training R²')
plt.ylabel('Test R²')
plt.title('Training vs Test Performance (Overfitting Check)')
...
table = plt.table(cellText=table_data,
                 colLabels=['Rank', 'Model', 'Test R²', 'Test MSE', 'CV R²'],
                 cellLoc='center',
                 loc='center',
                 bbox=[0, 0, 1, 1])
table.auto_set_font_size(False)
table.set_fontsize(10)
plt.title('Top 5 Models Performance Summary', pad=20)

plt.tight_layout()
plt.savefig('comprehensive_model_comparison.png', dpi=300, bbox_inches='tight')
plt.close()

# ハイパーパラメータ最適化（最良モデルに対して）
print("\n" + "="*60)
print("ハイパーパラメータ最適化（最良モデル）")
print("="*60)
...
if 'LightGBM' in best_model_name:
    ...
    print("グリッドサーチを実行中（並列処理）...")
    grid_search = GridSearchCV(
        lgb_model, param_grid, cv=3, 
        scoring='r2', n_jobs=n_jobs, verbose=1
    )
    
    if 'PCA' in best_model_name:
        grid_search.fit(X_train_pca, y_train)
        best_test_score = grid_search.score(X_test_pca, y_test)
    else:
        grid_search.fit(X_train_scaled, y_train)
        best_test_score = grid_search.score(X_test_scaled, y_test)
    
    print(f"\n最適化後のベストパラメータ: {grid_search.best_params_}")
    print(f"最適化後のテストR²: {best_test_score:.4f}")
...
print("  - model_performance_summary.csv: モデル性能のサマリー表")
print(f"\n最終結論:")
...
print(f"  - 総処理時間: {total_elapsed_time:.2f}秒（{n_jobs}コア使用）")